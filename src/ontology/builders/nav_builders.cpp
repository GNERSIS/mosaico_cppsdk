...
void MotionStateBuilder::initSchema() {
    schema_ = arrow::schema({
        ...
        arrow::field("pose", msco::poseType()),
        arrow::field("velocity", msco::velocityType()),
        arrow::field("target_frame_id", arrow::utf8()),
        arrow::field("acceleration", msco::accelerationType(), /*nullable=*/true),
...
arrow::Status MotionStateBuilder::appendRow(const FieldMap& fields) {
    ...
    ARROW_RETURN_NOT_OK(msco::appendPoint3d(pos_sb, fields, pose_prefix + "position."));
    // orientation inside pose
    auto* ori_sb = static_cast<arrow::StructBuilder*>(pose_sb->field_builder(1));
    ARROW_RETURN_NOT_OK(msco::appendQuaternion(ori_sb, fields, pose_prefix + "orientation."));
    // pose header
    ...
    ARROW_RETURN_NOT_OK(msco::appendCovariance(pose_cov, pose_ct, fields, "pose.covariance"));
    ...
    ARROW_RETURN_NOT_OK(msco::appendVector3d(lin_sb, fields, twist_prefix + "linear.", "", true));
    auto* ang_sb = static_cast<arrow::StructBuilder*>(vel_sb->field_builder(1));
    ARROW_RETURN_NOT_OK(msco::appendVector3d(ang_sb, fields, twist_prefix + "angular.", "", true));
    // velocity header
    ...
    ARROW_RETURN_NOT_OK(msco::appendCovariance(vel_cov, vel_ct, fields, "twist.covariance"));
