...
void decodeScalarValue(const gpb::Message& msg,
  ...
  switch (field->cpp_type()) {
    ...
    case gpb::FieldDescriptor::CPPTYPE_MESSAGE: {
      ...
      decodeMessage(sub_msg, field->message_type(), key, out);
      return;
    }
  }
}
  ...
  switch (field->cpp_type()) {
    ...
    case gpb::FieldDescriptor::CPPTYPE_MESSAGE: {
      ...
      for (int i = 0; i < count; ++i) {
        ...
        FieldMap elem;
        decodeMessage(sub_msg, field->message_type(), "", elem);
...
void decodeField(const gpb::Message& msg,
  ...
  if (field->is_repeated()) {
    decodeRepeatedField(msg, refl, field, key, out);
    return;
  }

  // Scalar field.
  decodeScalarValue(msg, refl, field, key, out);
}
...
void decodeMessage(const gpb::Message& msg,
  ...
  for (int i = 0; i < desc->field_count(); ++i) {
    decodeField(msg, desc->field(i), prefix, out);
  }
}
...
arrow::Status ProtobufDecoder::decode(
  ...
  decodeMessage(*msg, message_desc_, "", out);
  return arrow::Status::OK();
}
